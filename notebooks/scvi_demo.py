...
checkpoint_file = 'lightning_logs/version_36/checkpoints/epoch=49-step=3150.ckpt'
...
overwrite=False
filepath = "tmp_data/adata_embedded.h5ad"
...
filepath = "tmp_data/adata_scvi_reconstructed.h5ad"
if not os.path.exists(filepath) or overwrite:
    #adata = None
    #added = False
...
filepath = "tmp_data/adata_scvi_reconstructed_raw_umap.h5ad"
if not os.path.exists(filepath) or overwrite:
    adata = NF.plot_raw_data(adata,filepath)
else:
    print("Reading file : {}".format(filepath))
    adata = sc.read(filepath)
filepath = "tmp_data/adata_scvi_reconstructed_raw_scvi_umaps.h5ad"
if not os.path.exists(filepath) or overwrite:
...
filepath = "tmp_data/adata_scvi_reconstructed_raw_scvi_umaps_layers.h5ad"
if not os.path.exists(filepath) or overwrite:
...
if config_dict['model']['model']['init_args']['batch_embedded']:
    ...
    plt.savefig("figures/batch_index.pdf")
    plt.clf()
    plt.close()
    ...
    plt.xticks([])
    plt.yticks([])
    plt.savefig("figures/batch_embeddings.pdf")
    plt.clf()
    plt.close()
