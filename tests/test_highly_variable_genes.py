...
def test_highly_variable_genes_top_n(num_genes_to_check: int):
    ...
    result = get_highly_variable_genes(gene_names=gene_names, mean=mean, var=var, n_top_genes=num_genes_to_check, n_bins=1)
...
def test_highly_variable_genes_cutoffs():
    ...
    mean = torch.tensor([1, 2, 3, 4, 5, 6], dtype=torch.float32)
    var = torch.tensor([1.6086e00, 2.2582e02, 9.8922e-02, 1.4379e01, 4.0901e-02, 9.9200e-01], dtype=torch.float32)
    result = get_highly_variable_genes(gene_names, mean, var, min_disp=0.2, max_disp=10, min_mean=0, max_mean=5, n_bins=1)
...
def test_hvg_matches_scanpy_cutoffs():
    ...
    our_df = get_highly_variable_genes(list(adata_norm.var_names), mean_t, var_t,
                                        min_disp=0.5, max_disp=np.inf, min_mean=0.0125, max_mean=3.0)
    sc_df = sc.pp.highly_variable_genes(adata_norm, flavor="seurat",
                                         min_disp=0.5, max_disp=np.inf, min_mean=0.0125, max_mean=3.0, inplace=False)
...
def test_hvg_batch_matches_scanpy(n_top_genes: int):
    ...
    our_df = get_highly_variable_genes(
        list(adata_norm.var_names), mean_t, var_t,
        n_top_genes=n_top_genes,
        batch_mean_bg=batch_mean_bg, batch_var_bg=batch_var_bg, batch_ids=batches,
    )
    sc_df = sc.pp.highly_variable_genes(adata_norm, flavor="seurat",
                                         n_top_genes=n_top_genes, batch_key=batch_col, inplace=False)
...
def test_hvg_batch_partial_args_raises():
    with pytest.raises(ValueError, match="must all be provided together"):
        get_highly_variable_genes(["g0", "g1"], torch.ones(2), torch.ones(2),
                                   batch_mean_bg=torch.ones(2, 2))


def test_hvg_batch_shape_mismatch_raises():
    with pytest.raises(ValueError):
        get_highly_variable_genes(["g0", "g1", "g2"], torch.ones(3), torch.ones(3),
