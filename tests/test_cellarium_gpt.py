...
def test_load_from_checkpoint_multi_device(tmp_path: Path):
    ...
    data = {
        ...
        "token_mask_nc_dict": {
            "gene_id": (np.broadcast_to(np.arange(c), (n, c)) < s).astype(np.float32),
            "gene_value": (np.broadcast_to(np.arange(c), (n, c)) < s).astype(np.float32),
            "gene_query_mask": (np.broadcast_to(np.arange(c), (n, c)) < s).astype(np.float32),
            "total_mrna_umis": (np.broadcast_to(np.arange(c), (n, c)) < s).astype(np.float32),
