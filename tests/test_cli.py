...
CONFIGS = [
    ...
    {
        "model_name": "imputation_scvi",
        "subcommand": "fit",
        "fit": {
            "model": {
                "model": {
                    "class_path": "cellarium.ml.models.ImputationModel",
                    "init_args": {
                        "n_batch": None,
                        "use_size_factor_key": False,
                        "encoder": {
...
@pytest.mark.parametrize(
    "config",
    CONFIGS,
    ids=[config.get("_display_id", config["model_name"]) + "-" + config["subcommand"] for config in CONFIGS],  # type: ignore[operator]
)
