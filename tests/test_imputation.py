...
def test_run():
    ...
    for batch in train_loader:
        ...
        assert isinstance(output["loss"], torch.Tensor)  # mypy
        assert isinstance(output["reconstruction_loss"], torch.Tensor)  # mypy
        assert isinstance(output["kl_divergence_z"], torch.Tensor)  # mypy
        assert isinstance(output["z_nk"], torch.Tensor)  # mypy
        ...
        assert torch.all(torch.isfinite(output["reconstruction_loss"])), "All reconstruction losses should be finite"
        assert torch.all(torch.isfinite(output["kl_divergence_z"])), "All KL divergences should be finite"
...
def test_masking_functionality(masking_probability: float):
    ...
    assert torch.all(masked_data[:, masked_positions] == 0.0), "Masked gene positions should be set to zero"
...
def test_mask_creation_and_application_methods():
    ...
    expected_masked = int(masking_probability * g)
    assert mask1.sum().item() == expected_masked, f"Should mask {expected_masked} genes, got {mask1.sum().item()}"
    ...
    assert torch.all(masked_x[:, masked_positions] == 0.0), "Masked positions should be zero"
