...
def test_batch_linear_regression():
    ...
    assert np.allclose(slope_b, slopes_scipy, atol=1e-6), (
        "Slopes do not match between batch and individual regressions."
    )
    assert np.allclose(intercept_b, intercepts_scipy, atol=1e-6), (
        "Intercepts do not match between batch and individual regressions."
    )
    assert np.allclose(r_squared_b, r_squared_scipy, atol=1e-6), (
        "R-squared values do not match between batch and individual regressions."
    )
