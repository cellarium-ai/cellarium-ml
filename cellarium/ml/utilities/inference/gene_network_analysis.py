...
def quantile_normalize_select(
    x: np.ndarray, y: np.ndarray, n_bins: int, top_k: int, min_x: int | None = None, max_x: int | None = None
):
    """
...
class GeneNetworkAnalysisBase:
    ...
    def process(
        ...
        if query_hv_top_k is not None:
            ...
            logger.info(
                f"Number of query genes after highly-variable filtering: {np.sum(self.mask_q)} / {len(self.mask_q)}"
            )
    ...
    def compute_spectral_dimension(self, offset: int = 2, n_lambda_for_estimation: int = 5) -> float:
        ...
        norm_p = 1.0 / (1e-9 + np.sqrt(self.a_pp.sum(0)))
