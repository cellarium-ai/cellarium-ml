...
class CellariumGPT(CellariumModel, PredictMixin, ValidateMixin):
    ...
    #     total_mrna_umis_downsampled_n: torch.Tensor,
    #     cell_type_n: np.ndarray,
    #     gene_prompt_size_n: torch.Tensor,
    #     gene_query_size_n: torch.Tensor,
    #     gene_query_nnz_size_n: torch.Tensor,
    # ) -> None:
    #     logits_nck_dict = self.predict(
    #         token_value_nc_dict=token_value_nc_dict,
    #         token_mask_nc_dict=token_mask_nc_dict,
    ...
    #     loss_n = torch.sum(loss_nc * label_weight_nc, dim=-1) / label_weight_nc.sum(dim=-1)
    #     nnz_label_weight_nc = (label_nc > 0).float() * label_weight_nc
    #     loss_nnz_n = torch.sum(loss_nc * nnz_label_weight_nc, dim=-1) / nnz_label_weight_nc.sum(dim=-1)

    #     df = pd.DataFrame(
    #         data={
    #             # "prompt_size": prompt_mask_nc.sum(-1).cpu(),
    #             # "query_size": label_weight_nc.sum(-1).cpu().int(),
    #             # "total_mrna_umis": total_mrna_umis_n.cpu(),
    #             # # "total_mrna_umis_downsampled": total_mrna_umis_downsampled_n.cpu(),
    #             # "cell_type": cell_type_n.cpu(),
    #             # "loss": loss_n.cpu(),
    #             # "query_size_nnz": nnz_label_weight_nc.sum(-1).cpu().int(),
    #             # "loss_nnz": loss_nnz_n.cpu(),
    #             "prompt_size": gene_prompt_size_n.cpu(),
    #             "query_size": gene_query_size_n.cpu().int(),
    #             "total_mrna_umis": total_mrna_umis_n.cpu(),
    #             "total_mrna_umis_downsampled": total_mrna_umis_downsampled_n.cpu(),
    #             "cell_type": cell_type_n.cpu(),
    #             "loss": loss_n.cpu(),
    #             "query_size_nnz": gene_query_nnz_size_n.cpu().int(),
    #             "loss_nnz": loss_nnz_n.cpu(),
    #         },
    #         index=obs_names_n,
    #     )
    #     postfix = batch_idx * trainer.world_size + trainer.global_rank
    #     output_path = f"/mnt/cellariumgpt-training-data/info_theor/out/cell_state_prefix_0_seed_3113024_all_measured/batch_{postfix}.csv"
    #     # output_path = f"/mnt/cellariumgpt-training-data/info_theor/out/cell_state_all_prefixes_seed_3112024_all_measured/batch_{postfix}.csv"
    ...
    #     gene_query_size_n: torch.Tensor,
    #     gene_query_nnz_size_n: torch.Tensor,
    #     measured_query_mask_nc: torch.Tensor,
    #     unmeasured_query_mask_nc: torch.Tensor,
    # ) -> None:
    #     logits_nck_dict = self.predict(
    #         token_value_nc_dict=token_value_nc_dict,
    #         token_mask_nc_dict=token_mask_nc_dict,
    ...
    #     key = "gene_value"
    #     logits_nck = logits_nck_dict[key]
    #     k = logits_nck.shape[2]
    #     value_k = torch.arange(k, dtype=torch.float32, device=logits_nck.device)
    #     assert isinstance(logits_nck, torch.Tensor)
    #     probs_nck = logits_nck.softmax(-1)
    #     mean_nc = probs_nck @ value_k
    #     measured_mean_n = (mean_nc * measured_query_mask_nc).sum(-1) / measured_query_mask_nc.sum(-1)
    #     unmeasured_mean_n = (mean_nc * unmeasured_query_mask_nc).sum(-1) / unmeasured_query_mask_nc.sum(-1)
    ...
    #             "total_mrna_umis_downsampled": total_mrna_umis_downsampled_n.cpu(),
    #             "cell_type": cell_type_n.cpu(),
    #             "measured_mean": measured_mean_n.cpu(),
    #             "measured_size": measured_query_mask_nc.sum(-1).cpu(),
    #             "unmeasured_mean": unmeasured_mean_n.cpu(),
    #             "unmeasured_size": unmeasured_query_mask_nc.sum(-1).cpu(),
    #             # "loss": loss_n.cpu(),
    #             # "query_size_nnz": gene_query_nnz_size_n.cpu().int(),
    #             # "loss_nnz": loss_nnz_n.cpu(),
    #         },
    ...
    #     )
    #     postfix = batch_idx * trainer.world_size + trainer.global_rank
    #     output_path = f"/mnt/cellariumgpt-training-data/info_theor/out/unmeasured/batch_{postfix}.csv"
    #     df.to_csv(output_path)
    ...
    def test(
        ...
        logits_nck = logits_nck_dict[key]
        probs_nck = logits_nck.softmax(-1)
        pred_label_nc = probs_nck.argmax(-1)
