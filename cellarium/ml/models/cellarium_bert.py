...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        labels_mask_nc = torch.bernoulli(torch.full(labels_mask_nc.shape, 0.9, device=device)).bool() & labels_mask_nc
...
class CellariumBERT(CellariumModel):
    """
    Cellarium GPT model.
    ...
    def tokenize(
        ...
        x_nc = x_ng[:, :genes_context_size]
        if shuffle:
            indices_nc = torch.argsort(torch.rand_like(x_nc))
        else:
            indices_nc = torch.arange(genes_context_size, device=device).expand(n, -1)
        values_nc = x_nc[ndx[:, None], indices_nc]
    ...
    def forward(
        ...
        hidden_state_ncd = self.transformer(ids_nc, values_nc, labels_mask_nc, None, "full")
    ...
    def predict(
        ...
        # prefix includes the total_mrna_umis and a random subset of genes
        # the length of the prefix is sampled uniformly from 1 to n_context - 1 (inclusive)
        labels_mask_nc = torch.zeros(n, self.n_context, dtype=torch.bool, device=device)

        ids_nc, values_nc = self.tokenize(x_ng, total_mrna_umis_n, self.n_context, shuffle=True)
        hidden_state_ncd = self.transformer(ids_nc, values_nc, labels_mask_nc, None, "full")

        return {
            "hidden_state_ncd": hidden_state_ncd[:, 1:],
            "var_names_g": var_names_g[: self.n_context - 1],
        }
